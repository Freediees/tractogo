...
const filterStock = (data, filter) => {
  const newArr = data.filter((newData) => {
    console.log({ newData })
    console.log({ filter })
    if (newData.discountedPrice) {
      if (
        (newData.discountedPrice >= filter.selectedMin) &&
        (newData.discountedPrice <= filter.selectedMax)
      ) {
      ...
      if (
        (newData.priceAmount >= filter.selectedMin) &&
        (newData.priceAmount <= filter.selectedMax)
      ) {
        console.log('filter.selectedMin', filter.selectedMin)
        console.log('filter.selectedMax', filter.selectedMax)
        console.log('newData.discountedPrice', newData.discountedPrice)
