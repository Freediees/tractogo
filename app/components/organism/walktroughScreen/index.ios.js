...
const slides = [
    ...
    },
    {
        ...
        text: 'Wherever and whenever you are ready, our driver always accessible through your phone to get you to your next destination!',
                        ...
                        {datasource.length > 1 &&
                            datasource.map((_, i) => (
                                <TouchableOpacity
                                    ...
                                    onPress={() => { this.AppIntroSlider.goToSlide(i, true) }}
                                />
                            ))}
                    </View>
    ...
    const _renderItem = ({ item }) => {
        return (
            <View style={styles.container}>
                <Image source={item.image} style={{ width: wp(100), height: hp(100), resizeMode: 'cover' }} />
                <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 997, height: hp(42), padding: '5%' }}>
                    <Text style={{ ...Fonts.f_20, color: Colors.white, paddingBottom: 20, width: '80%' }}>{item.title}</Text>
...
WalktroughScreen.defaultProps = {
    datasource: slides,
    labelSignUp: 'Daftar',
    labelSignin: 'Saya telah mempunyai akun',
    onSignUp: () => { },
    onSignIn: () => { }
}
